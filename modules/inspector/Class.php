...
class InspectorClass extends InspectorReflectorCommented implements InspectorReflectorInterface {
  ...
  protected function loadAliases($sContent) {

    if (preg_match('/^use(\s*[\w\\\,\s]*)/m', $sContent, $aResult)) {

      $aNamespaces = explode(',', $aResult[1]);

      foreach ($aNamespaces as $sNamespace) {

        $sNamespace = trim($sNamespace);

        $iName = strrpos($sNamespace, '\\');
        $sName = substr($sNamespace, $iName + 1);

        if ($sNamespace{0} != '\\') $sNamespace = '\\' . $sNamespace;

        $this->aAlias[$sName] = $sNamespace;
      }
      //foreach ($aResult)
    }
  }

  protected function getAlias($sPrefix) {

    if (!array_key_exists($sPrefix, $this->aAlias)) {

      $this->throwException(txt('Alias %s unknown', $sPrefix));
    }

    return $this->aAlias[$sPrefix];
  }

  public function completeClass($sClass) {

    $sResult = $sClass;

    if ($sClass && $sClass{0} != '\\' && $sClass != 'array') {

      if ($iName = strpos($sClass, '\\')) {

        $sPrefix = substr($sClass, 0, $iName);
        $sAlias = $this->getAlias($sPrefix);
      }
      else {

        $sAlias = '\\' . $this->getReflector()->getNamespaceName() . '\\';
      }

      $sResult = $sAlias . substr($sClass, $iName);
    }
  ...
  protected function loadParents() {
    ...
    preg_match('/' . str_replace('\\', '\\\\', $this->getName()) . '(?:\s+extends\s+(?P<extends>\w+))?(?:\s+implements\s+(?P<implements>[\w,\s]+))?/', $aSource[0], $aMatch);
